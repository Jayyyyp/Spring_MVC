...
public class ScoreRepositoryTest {
    ...
    @DisplayName("저장소에서 2번 학생 삭제 후 리스트 전체 조회시 개수는 2개, 다시 2번 학생 조회는 Null")
    @Test
    public void deleteTest(){
        // given 학생 번호를 저장한다.
        int studentNumber = 2;

        // when 해당 번호 학생을 삭제한다. 그리고 findAll()로 전체 데이터를 가져오고, 2번 학생만 가져오기도 다시한다.
       boolean result =  repository.deleteByStudentNumber(studentNumber);
        ...
        // then 전체목록의 길이는 2일 것이고, score 변수는 null이 담긴다고 단언
        // assertEquals(2, scoreList.size());
        assertThat(scoreList.size()).isEqualTo(2);
        assertNull(score);
        assertTrue(result); // 삭제성 공시 true
    }
