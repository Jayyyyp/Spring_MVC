...
public class MVCController01 {
    ...
    */
    ...
    public String computer(NotebookDTO computer){
        System.out.println("파라미터로 전달받은 computer 사양 : " + computer);
        return "";
    }

    @RequestMapping("/dog")
    public String dog(DogDTO dog){
        System.out.println("강아지 정보 : " + dog);
        return "";
    }
/*
...
*/
    ...
    public String board(@PathVariable int bNo){ // 왼쪽에 @PathVariable을 붙여준다
        System.out.println("경로로 받아온 bNo의 값 : " + bNo);
        return "";
    }

    // /member/회원이름/회원번호
    // 패턴으로 파라미터를 처리하도록 만들기
    // 회원 이름은 mName, 회원번호는 mNo명칭 사용하기
    ...
    // 데이터를 보낼때는 URL에 직접 파라미터를 붙여서 보내게 하는 대신
    ...
    public String cat(String name, int age, String kind){
        System.out.println("등록한 고양이 이름 : " + name);
        System.out.println("등록한 고양이 나이 : " + age);
        System.out.println("등록한 고양이 종류 : " + kind);
        // chap01 폴더 내부의 catreq.jsp로 연결되게 하기
        return "/chap01/catreg";
    }
    // 아이디, 비밀번호를 각각 id, pw라는 name에 실어서 전달해주는 폼을 생성하기
    // chap01 폴더 내에 loginform.jsp로 지정하기
    ...
    @RequestMapping(value = "/login", method = RequestMethod.POST) // 보낸 아이디와 비밀번호를 화면에 찍기
